...
export default function AICopilot({ context, onUpdateCalculator }) {
    ...
    const containerClasses = "fixed z-[10000] flex flex-col items-end gap-4 transition-all duration-300 bottom-28 right-4 md:bottom-6 md:right-6";
    const windowClasses = "bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col transition-all origin-bottom-right w-[calc(100vw-2rem)] md:w-96 h-[500px] max-h-[60vh] md:max-h-[500px]";
                            ...
                            <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                <div className={`max-w-[85%] p-3 rounded-2xl text-sm ${
                                    m.role === 'user' 
                                        ...
                                        : m.role === 'system'
                                            ? 'bg-emerald-50 text-emerald-700 border border-emerald-100 text-xs italic'
