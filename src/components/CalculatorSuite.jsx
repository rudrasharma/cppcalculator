...
const TABS = [
  { 
    id: 'cpp', 
    label: 'CPP & OAS', // Clearer than "Retirement"
  ...
  { 
    id: 'parental', 
    label: 'Mat. Leave', // Clearer than "Parental"
  ...
  { 
    id: 'ccb', 
    label: 'Family Cash', // "Benefits" is too vague
...
export default function CalculatorSuite() {
  ...
  // Derived state for the Header Title
  const activeTabInfo = TABS.find(t => t.id === view) || TABS[0];
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-24 md:pb-0"> 
      ...
      {/* MOBILE HEADER - NOW WITH CONTEXT! */}
      <nav className="md:hidden bg-white/90 backdrop-blur-xl border-b border-slate-200 sticky top-0 z-40 px-4 py-3 flex flex-col items-center justify-center text-center">
         ...
         {/* DYNAMIC TITLE: Tells user exactly what this tool is */}
         <div className="animate-fade-in key={view}">
             <h1 className="text-xs font-black uppercase tracking-widest text-slate-400">{activeTabInfo.title}</h1>
         </div>
      </nav>

      {/* DESKTOP HERO TITLE AREA (Optional but recommended) */}
