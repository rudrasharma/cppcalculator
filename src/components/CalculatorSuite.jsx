...
// ICONS (Simple SVG placeholders - replace with your icon set)
...
export default function CalculatorSuite() {
  ...
  // 1. URL SYNCING LOGIC (Bi-directional)
  useEffect(() => {
    // On Mount: Read URL
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-24 md:pb-0"> {/* Padding bottom for mobile nav */}
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 pb-safe z-50 flex justify-around items-center h-16 px-2 shadow-[0_-5px_20px_-5px_rgba(0,0,0,0.05)]">
        {TABS.map((tab) => (
          <button
            key={tab.id}
            onClick={() => {
