                            ...
                            <label className="flex items-center text-sm font-medium text-slate-700 mb-2 gap-2">
                                ...
                                <div className="group relative flex items-center">
                                    ...
                                    <div className="absolute bottom-full mb-2 left-0 w-64 p-2 bg-slate-800 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10 shadow-xl font-normal text-left">
                                        Used to calculate tax-free Canada Child Benefit (CCB) payouts if you retire early while children are under 18.
                                    </div>
                                </div>
                            </label>
                            <div className="flex flex-wrap gap-2 items-center">
                                {state.children?.map((year, i) => (
                                    <div key={i} className="flex items-center bg-slate-50 border border-slate-200 rounded-xl overflow-hidden">
                                        <span className="px-2 py-2 text-xs font-bold text-slate-400 border-r border-slate-200 bg-slate-100">Child {i + 1}</span>
                                        <input 
                                            type="number" 
                                            value={year} 
                                            onChange={(e) => {
                                                const newChildren = [...(state.children || [])];
                                                newChildren[i] = parseInt(e.target.value) || 0;
                                                updateField('children', newChildren);
                                            }}
                                            className="w-20 p-2 text-sm outline-none text-center bg-white text-slate-700" 
                                            placeholder="YYYY" 
                                        />
                                        <button 
                                            onClick={() => updateField('children', state.children.filter((_, index) => index !== i))}
                                            className="px-2 py-2 text-rose-400 hover:text-rose-600 hover:bg-rose-50 transition-colors"
                                        >
                                ...
                                <button 
                                    onClick={() => updateField('children', [...(state.children || []), new Date().getFullYear()])}
                                    className="px-3 py-2 text-sm font-semibold text-indigo-600 bg-indigo-50 border border-indigo-100 rounded-xl hover:bg-indigo-100 transition-colors"
                                >
                                    + Add Child
                                </button>
                            </div>
