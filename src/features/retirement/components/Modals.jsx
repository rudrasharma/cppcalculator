        ...
        >
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden relative">
                <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                    <h2 id="about-title" className="text-lg font-black text-slate-800">About LoonieFi</h2>
                    <button 
                        onClick={onClose} 
...
// --- IMPORT MODAL (Accessible Update) ---
            ...
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                    <div className="flex items-center gap-3">
                        <div className={`p-2.5 rounded-xl ${previewData ? 'bg-emerald-100 text-emerald-600' : 'bg-indigo-100 text-indigo-600'}`} aria-hidden="true">
                            {previewData ? <CheckIcon size={24} strokeWidth={3} /> : <UploadIcon size={24} />}
                        </div>
                        <div>
                            <h2 id="import-title" className="text-xl font-black text-slate-800 leading-none">
                    ...
                    >
                        {XIcon && <XIcon size={24} aria-hidden="true" />}
                    </button>
                </div>
                    ...
                    {!previewData ? (
                        ...
                        <>
                            <div className="bg-indigo-50 rounded-2xl p-5 border border-indigo-100 mb-6">
                                <h3 className="text-indigo-900 font-bold text-sm mb-3 uppercase tracking-wide flex items-center gap-2">
                                    <span className="bg-indigo-600 text-white w-5 h-5 rounded-full flex items-center justify-center text-[10px]" aria-hidden="true">!</span> 
                                    How to Copy Correctly
                                </h3>
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-indigo-800">
                                    <div className="bg-white p-3 rounded-xl border border-indigo-100 shadow-sm">
                                        <span className="block text-[10px] font-bold text-indigo-500 uppercase tracking-widest mb-1">Step 1</span>
                                    ...
                                    <div className="bg-white p-3 rounded-xl border border-indigo-100 shadow-sm relative overflow-hidden">
                                        <span className="block text-[10px] font-bold text-indigo-500 uppercase tracking-widest mb-1">Step 2 (Crucial)</span>
                                    ...
                                    <div className="bg-white p-3 rounded-xl border border-indigo-100 shadow-sm">
                                        <span className="block text-[10px] font-bold text-indigo-500 uppercase tracking-widest mb-1">Step 3</span>
                            ...
                            <div className="space-y-3">
                                <label htmlFor="import-textarea" className="text-sm font-bold text-slate-700 ml-1">Paste Data Here:</label>
                                <textarea 
                                    id="import-textarea"
                                    value={importText} 
                                    onChange={(e) => setImportText(e.target.value)} 
                                    className="w-full h-48 p-4 bg-slate-50 border border-slate-300 rounded-2xl text-xs font-mono focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all resize-none shadow-inner text-slate-900"
                                    placeholder="Paste the entire text content of the page here..."
                                />
                                {localError && (
                                    <div className="flex items-center gap-3 text-rose-600 bg-rose-50 p-4 rounded-xl border border-rose-100 animate-slide-in" role="alert">
                                        {InfoIcon && <InfoIcon size={20} className="shrink-0" aria-hidden="true" />}
                        ...
                        <div className="animate-fade-in space-y-6">
                            ...
                            <div className="grid grid-cols-3 gap-4">
                                ...
                                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 text-center">
                                    <div className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Start</div>
                                    <div className="text-2xl font-black text-slate-800">{minYear}</div>
                                </div>
                                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 text-center">
                                    <div className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">End</div>
                            ...
                            <div className="bg-slate-50 rounded-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[300px]">
                                <div className="p-3 bg-slate-100 border-b border-slate-200 grid grid-cols-12 font-bold text-xs text-slate-600 uppercase tracking-wider items-center">
                                    ...
                                    {yearsFound.map(year => (
                                        <div key={year} className="grid grid-cols-12 py-1.5 px-2 border-b border-slate-100 last:border-0 text-sm hover:bg-white rounded-lg transition-colors items-center group">
                                            ...
                                            <div className="col-span-7 relative">
                                                <label htmlFor={`earnings-${year}`} className="sr-only">Earnings for {year}</label>
                                                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 text-xs font-mono" aria-hidden="true">$</span>
                                                <input 
                                                    id={`earnings-${year}`}
                                                    type="number"
                                                    value={previewData[year]}
                                                    onChange={(e) => handleEditEarnings(year, e.target.value)}
                                                    className="w-full pl-6 pr-3 py-1.5 bg-white border border-slate-200 rounded-lg text-sm font-mono focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all shadow-sm text-slate-900"
                                                />
                                            </div>

                                            {/* Delete Button (Accessible Update) */}
                                            <div className="col-span-2 flex justify-center">
                                                <button 
                                                    onClick={() => handleDeleteRow(year)}
                                                    className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors"
                                                    title="Remove Row"
                                                    aria-label={`Remove earnings for year ${year}`}
                                                >
                ...
                <div className="p-4 bg-slate-50 border-t border-slate-200 flex justify-end gap-3">
                    ...
                    {!previewData ? (
                        ...
                        >
                            {FileTextIcon && <FileTextIcon size={18} aria-hidden="true" />} Analyze Text
                        </button>
                    ) : (
                        <>
                            <button 
                            ...
                            >
                                {RotateCcwIcon && <RotateCcwIcon size={18} aria-hidden="true" />} Back
                            </button>
                            <button 
                                onClick={handleConfirm} 
                                className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl shadow-lg shadow-emerald-200 transition-all flex items-center gap-2 animate-pulse-soft"
                            >
                                {CheckIcon && <CheckIcon size={18} aria-hidden="true" />} Confirm Import
                            </button>
                        </>
                    )}
                </div>
